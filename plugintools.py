...
def read_online_csv(url): #not used in plugin.video.heritagechannel, as only local csv is used
    ...
    try:
        ...
        xbmcgui.Dialog().ok("Please check URL (HTTPError)",
                            "Database at github.com is unavailable. [CR]Using local database instead.")
        return False
    except urllib2.URLError, e:
        xbmcgui.Dialog().ok("Please check URL (URLError)",
                            "Database at github.com is unavailable. [CR]Using local database instead.")
        return False
    except ValueError:
        xbmcgui.Dialog().ok("Please check URL (ValueError)",
                            "Database at github.com is not unavailable. [CR]Using local database instead.")
        return False
    except Exception:
        xbmcgui.Dialog().ok("An error occured (Exception)",
                            "Database at github.com is unavailable. [CR]Using local database instead.")
        return False
    else:
        list = []
...
def read_local_csv(filepath):
    ...
    try:
        ...
        xbmcgui.Dialog().ok("I/O error({0}): {1}".format(errno, strerror), "Check filepath of local database", filepath)
        sys.exit()
    except Exception:
        xbmcgui.Dialog().ok("An error occured (Exception)", "Expected: resources/data/institutions.csv")
        sys.exit()
    else:
        list = []
...
def lookup_countryname(countrycode):
    ...
    try:
        ...
        xbmcgui.Dialog().ok("I/O error({0}): {1}".format(errno, strerror), "Check filepath of countrydata.csv",
                            url_country)
        sys.exit()
    except Exception:
        xbmcgui.Dialog().ok("An error occured (Exception)", "Expected: resources/data/countrydata.csv")
        sys.exit()
    else:
        a = "Wrong country code!!"
        response = open(url_country, 'r')
        sheet = csv.reader(response, delimiter=';')
        for row in sheet:
            # TODO: convert to dictionary, so we can call by name instead of id, thus be more flexible csv-wise (toch?)
    ...
    if cj is not None:
        ...
        if cookielib is not None:
            ...
            if not follow_redirects:
                opener = urllib2.build_opener(urllib2.HTTPHandler(debuglevel=http_debug_log_enabled),
...
def add_item(action="", title="", plot="", url="", thumbnail="", fanart="", show="", episode="", extra="", page="",
             info_labels=None, isPlayable=False, folder=True):
    _log(
    ...
    if url.startswith("plugin://"):
        ...
        sys.argv[0], action, urllib.quote_plus(title), urllib.quote_plus(url), urllib.quote_plus(thumbnail),
        urllib.quote_plus(plot), urllib.quote_plus(extra), urllib.quote_plus(page))
        xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=itemurl, listitem=listitem, isFolder=folder)
    else:
        itemurl = '%s?action=%s&title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&page=%s' % (
        sys.argv[0], action, urllib.quote_plus(title), urllib.quote_plus(url), urllib.quote_plus(thumbnail),
